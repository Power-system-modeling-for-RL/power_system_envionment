...
macmax = max(mac_con(:,1));  %% machine number
...
%em has no xd or xq
mac_em_idx = find(mac_con(:,6)==0);  %    mac_con(:,6)   6. d-axis sychronous reactance x_d(pu),
...
%sub has xdpp
mac_sub_idx = find(mac_con(:,8)~=0);  %  mac_con(:,8)  d-axis subtransient reactance x"_d(pu),
