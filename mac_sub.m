...
%
% Purpose: voltage-behindmac_and-subtransient-reactance generator
...
if n_sub~=0
    if flag == 0; % initialization
        if i ~= 0
            ...
            if ~isempty(notpp_idx)
                ...
                disp('xqp made equal to xqpp at generators  '); disp((mac_sub_idx(uets_idx))')
            end
    ...
    if flag == 2 || flag == 3 % generator dynamics calculation
        if i ~= 0
            ...
            dmac_spd(mac_sub_idx,k) = (pmech(mac_sub_idx,k)+ pm_sig(mac_sub_idx,k)-Te-mac_con(mac_sub_idx,17).*(mac_spd(mac_sub_idx,k)-ones(n_sub,1))-mac_con(mac_sub_idx,18).*(mac_spd(mac_sub_idx,k)-sys_freq(k)*ones(n_sub,1)))./(2*mac_con(mac_sub_idx,16));      
